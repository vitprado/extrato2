...
public class CarregamentoJpaController implements Serializable {
	...
	public List<Carregamento> findOrdemDistinct() {
		EntityManager em = getEntityManager();
		try {
			Query query = em.createNativeQuery("select DISTINCT RDC_ORDEM from CARREGAMENTO");
			return query.getResultList();
		} finally {
			em.close();
		}
	...
	public List<Carregamento> findCarregamentos(CarregamentoJpaFilter filter) {
		...
		try {
			...
			if (filter.getDataInicio() != null) {
				...
				stringBuilder.append(format(" and RDC_DATAJSON >= '%s' ", dataInicio));
			}

			if (filter.getDatafim() != null) {
				String dataFim = dateTimeFormatter.format(filter.getDatafim());
				stringBuilder.append(format(" and RDC_DATAJSON <= '%s' ", dataFim));
			}
